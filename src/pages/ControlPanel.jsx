...
const ControlPanel = () => {
  ...
  const updateStatus = (index, newStatus) => {
    ...
    updatedPipelines[index].end = new Date().toLocaleTimeString();
              ...
              <tr key={index} className="border border-gray-300 text-center">
                ...
                <td className="p-2 flex gap-2 justify-center">
                  <button
                    className={`bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 ${
                      pipeline.status !== "Running" && "cursor-not-allowed"
                    }`}
                    onClick={() => updateStatus(index, "Completed")}
                    disabled={pipeline.status === "Running" ? false : true}
                  >
                    Refresh
                  </button>
                  <button
                ...
                <td className="p-2">
                  <button
                    ...
                    onClick={() => {
                      setMessageContent(
                        "The Pipeline Status Is " + pipeline.status
                      );
                      setShowMessage(true);
                    }}
                  >
